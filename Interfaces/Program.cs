...
class ProductionDateTimeProvider :IDateTimeProvider
{
...
{
	public DateTime UtcNow => new DateTime(2023, 02, 02);
}


class SomeTests
{
	void LetsDoSomeTest(IDateTimeProvider provider)
	{
		var fiscalizationDate = provider.UtcNow;
	}

	bool StartTests()
	{
		IDateTimeProvider dateTimeProvider = new TestDateTimeProvider();
		LetsDoSomeTest(dateTimeProvider);
